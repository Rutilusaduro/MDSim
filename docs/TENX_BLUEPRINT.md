...
**Accept:** header contains zero emoji, zero gradient, and the stamp reads correctly for all seven cover strings (`getCoverLabel` has five bands, and the top two bands carry roster-warming variants).
...
**Do:** run census after D1–D6; for every pool in the top-100 exposure list with <6 lines, deepen to 6–10 lines *unevenly* (per voice cards; some pools get 3 sharp lines instead — asymmetry). Complete the tone grid: 4 tones × 3 tone-enabled actions × 4 tiers of narrative coverage (today `visitTones.js` carries reply snippets only; `buildToneNarrative` leans on the toneless pools).
**Budget:** ~450 strings.
...
### The crosswalk — where the older plans landed

This repo already holds four planning generations, and an implementer deserves one table saying which promises this blueprint inherits, so nothing gets built twice. `UP-Dn` refers to `THE_ULTIMATE_PLAN.md` §2's verdict table; `FM-n` to the five amendments in `FABLE_REVIEW_MEMO.md` §5.

| Older commitment | Status | Lives here as |
|---|---|---|
| UP-D1 weigh-in as a decision / FM-1 weigh-in ritual | shipped (chart choice exists) | B7 stages it; D1 gives it a voice |
| UP-D2 dual ledger, UP-D4 memory ledger | shipped | consumed everywhere; protected by F5's keep-priority |
| UP-D3 confession from the player's lies / FM-2 tier ceremonies | shipped generic | D2 scales it to families; E5 makes its gate executable |
| FM-3 refusals and her-led beats | never built | D3, whole |
| UP-D5/D15 mole as romance, telegraphed leak | shipped | untouched here; C3's cover ops interact with its heat math — retune in S4 |
| UP-D7 authored rival mirror (returns heavier) | never built | D8.2 |
| UP-D10/D11 whispers + pre-warmed referrals | never built | D6.2–D6.3 |
| UP-D12 clinic-as-character, UP-D13 appetite weather | shipped | B4 puts the tagline on the letterhead; D5 gives the weather more sky |
| UP-D14 complicity is cover | never built | C4 |
| UP-D16 shadow-life letters | never built | D8.1 |
| UP-D18 epilogue matrix (stage × mindset × gap) | never built | D9.2 |
| FM-5 consent surfaces / heat-band audit | half-built (`heatCap` in state, no UI) | F4 |

Everything in the ULTIMATE_PLAN marked DEFERRED (contagion math, clinic-label teeth, style-drift coupling) stays deferred; its trigger conditions stand unmodified.
...
**Do:** three save fixtures, committed as JSON: `fresh` (createNewGame with a fixed seed), `mid` (10 weeks), `endgame` (30 weeks). The mid/endgame fixtures are produced in S0 by a ten-line driver in `tests/helpers.js` that loops `endWeek` directly — the full sim (C6) doesn't exist yet, and this must not wait for it. When C6 lands, regenerate them once from the balanced policy and freeze.
**Accept:** `npm test` runs green with one placeholder test; fixtures load through `normaliseState` without defaulting any field.
